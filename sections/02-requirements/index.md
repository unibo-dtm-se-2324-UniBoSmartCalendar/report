...
This subsection is included in order to define potentially ambiguous or complex terms used throughout the requirements. The aim is to ensure that all requirements are interpreted consistently by both developers and evaluators.
...
This subsection describes the **observable behavior** of the application from the user and system perspectives (what the system does). Functional requirements are expressed through user stories and feature statements, each paired with **acceptance criteria** to make the expected outcome concrete.
...
This subsection defines constraints that do not directly describe features, but rather the **quality attributes** of the system, such as performance, reliability, compatibility, and testability. These requirements ensure the application is usable and robust in realistic conditions.
...
This subsection specifies the **constraints on the development and realization** of the system (technology stack, tools, testing/CI expectations). These requirements ensure consistency across environments and alignment with the project’s chosen engineering approach.
...
## 2.6 Risk Assessment (Severity / Urgency)

The following table summarizes the main project risks, rated by **Severity** (impact on the product) and **Urgency** (how soon the risk may materialize / how quickly it should be addressed). Each risk is also assigned to a **Risk Owner** responsible for monitoring and mitigation actions.

| ID | Risk | Severity | Urgency | Risk Owner | Mitigation / Countermeasure |
|---:|------|:--------:|:-------:|:----------:|-----------------------------|
| R1 | UniBo source changes (ICS/JSON structure or URLs) break parsing or feed generation | High | High | Alessandro | Centralize parsing/proxy logic; add validation checks; create/update tests for edge cases; document how to update sources |
| R2 | External calendar interoperability issues (Google/Outlook/Apple interpret ICS differently) | High | Medium | Alessandro | Keep ICS minimal and standards-compliant; test export/subscription on multiple clients; document known limitations |
| R3 | API/network failures prevent schedule retrieval | Medium | High | Simone | Clear UI error states + retry policy; timeouts; monitoring via CI checks; (optional) cache last successful schedule |
| R4 | Conflict detection logic produces false positives/negatives | Medium | Medium | Simone | Unit tests for overlap/adjacent cases; define conflict semantics clearly; validate with sample schedules |
| R5 | Performance issues when rendering many events (UI lag) | Medium | Medium | Giulia | Optimize rendering (memoization, reduced re-renders); avoid unnecessary recalculations; cache computed metadata client-side when possible |
| R6 | CI pipeline instability or environment mismatch (Node/npm versions) | Medium | Medium | Simone | Pin Node version in CI; document prerequisites; keep scripts aligned; run build in CI on every PR |
| R7 | Deployment misconfiguration (routing/API rewrites on Vercel) breaks subscription URL or endpoints | High | Medium | Simone | Document deployment steps; verify production routes; keep rewrites versioned; add smoke checks after deploy |
| R8 | Incomplete documentation reduces usability for evaluators/users | Low | Medium | Giulia | Keep report/README updated; include setup + test screenshots; provide short, verifiable instructions |
